...
button_classes = 'm-2 p-2 text-blue-700 bg-white hover:bg-red-200 hover:text-red-500 border focus:border-red-500 focus:outline-none'
message_classes = 'ml-4 p-2 text-lg bg-blue-500 text-white overflow-auto font-mono rounded-lg'

shared_div = jp.Div(classes='m-2 h-1/2 border overflow-auto', delete_flag=False)
header = jp.Div(text='Fake News Detector', classes='bg-red-100 border-l-4 border-red-500 text-red-700 p-4 text-3xl', delete_flag=False)
...
def message_initialize():
    ...
    p = jp.Pre(text='Test for Fake News', classes=message_classes)
...
def fake_test():
    ...
    message = jp.Textarea(placeholder='Paste the Tested Sentence', a=d, classes=input_classes, debounce=500)
...
jp.justpy(fake_test, startup=message_initialize)